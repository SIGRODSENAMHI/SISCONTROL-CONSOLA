...
public class FunctionDaoImpl extends AccesoDB implements FunctionDao{
	...
	public List<FunctionBean> obtNameFunction(String nvar, Integer opcion) throws Exception {
		...
		try {
			...
			LOGGER.error("Error FunctionDaoImpl.obtNameFunction " + e.getMessage());
		} finally {
			cerrarConexionDS();
			stmt.close();
			rs.close();
	...
	public String obtFechaStringBD() {
		...
		try {
			...
			abrirConexionDS();
			consulta = "SELECT TO_CHAR(SYSDATE,'DD/MM/YYYY') AS FECHA FROM  DUAL";
...
//			stmt.setFetchSize(1000);
			...
			LOGGER.error("Error FunctionDaoImpl.obtFechaStringBD " + e.getMessage());
		} finally {
			cerrarConexionDS();
	...
	public String obtHoraStringBD() {
		...
		try {
			abrirConexionDS();
			consulta = "SELECT TO_CHAR(SYSDATE,'HH24:MI:SS') AS HORA FROM  DUAL";
			stmt = ds.createStatement();
			rs = stmt.executeQuery(consulta);
			while (rs.next()) {
				HoraString = rs.getString("HORA");
			}
			stmt.close();
			rs.close();
		} catch (Exception e) {
			LOGGER.error("Error FunctionDaoImpl.obtHoraStringBD" + e.getMessage());
		} finally {
			cerrarConexionDS();
		}
		return HoraString;
